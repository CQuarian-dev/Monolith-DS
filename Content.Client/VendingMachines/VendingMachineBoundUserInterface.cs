...
namespace Content.Client.VendingMachines
{
    ...
    {
        [ViewVariables]
        private LuaVendingMachineWindow? _menu; // LuaM
        ...
        protected override void Open()
        {
            ...
            _menu = this.CreateWindowCenteredLeft<LuaVendingMachineWindow>(); // LuaM
        ...
        public void Refresh()
        {
            if (_menu == null || !EntMan.HasComponent<VendingMachineComponent>(Owner))
                return; // исправлен древний баг торгоматов из-за которых при открытии окна
            ...
            _menu?.Populate(_cachedInventory, enabled, _mod, _balance, cashSlotValue, _requiresCash); // Frontier: add _balance, mono: add _requiresCash
        }

        private void OnItemSelected(InventoryType type, string id) // LuaM
        {
